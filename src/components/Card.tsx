...
Card.Title = function CardTitle<T extends React.ElementType = 'h2'>({
  ...
  return (
    <Component className="text-base font-semibold tracking-tight text-zinc-800 dark:text-zinc-100">
      {href ? <Card.Link href={href} target="_blank" rel='noopener'>{children}</Card.Link> : children}
    </Component>
  )
}
...
Card.Cta = function CardCta({ children }: { children: React.ReactNode }) {
  return (
    <div
      aria-hidden="true"
      className="relative z-10 mt-4 flex items-center text-sm font-medium text-purple-500 dark:text-purple-400"
    >
      {children}
